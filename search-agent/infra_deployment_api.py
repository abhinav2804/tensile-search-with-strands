...
def run_docker_compose(compose_file, instance_id):
    ...
    try:
        ...
        # Wait for init service to complete and get API key
        time.sleep(30)  # Wait for init to complete
        api_key = get_generated_api_key(instance_id)
        
        if api_key:
            # Update compose file with API key
            update_compose_with_api_key(compose_file, instance_id, api_key)
        ...
        if instance_id in active_deployments:
            ...
            if api_key:
                active_deployments[instance_id]['elasticsearch_api_key'] = api_key
...
def get_generated_api_key(instance_id):
    ...
    try:
        ...
        cmd = ['docker', 'logs', f'elasticsearch-init-{instance_id}']
        result = subprocess.run(cmd, capture_output=True, text=True)
        
        if result.returncode == 0:
            # Extract encoded API key from logs
            for line in result.stdout.split('\n'):
                if 'Generated Encoded API Key:' in line:
                    api_key = line.split('Generated Encoded API Key:')[1].strip()
                    print(f"Found encoded API key for {instance_id}: {api_key}")
                    return api_key
...
def update_compose_with_api_key(compose_file, instance_id, api_key):
    """Update docker-compose file with the generated API key"""
    try:
        with open(compose_file, 'r') as f:
            content = f.read()
        
        # Add ES_API_KEY environment variable to MCP server
        ...
        print(f"Updated compose file {compose_file} with API key")
        return True
...
def update_mcp_with_api_key(instance_id):
    ...
    try:
        ...
        # Update the docker-compose file with the API key
        with open(compose_file, 'r') as f:
            content = f.read()
        
        # Replace the ES_API_KEY placeholder with the actual key
        updated_content = content.replace('ES_API_KEY: ${ES_API_KEY:-}', f'ES_API_KEY: {api_key}')
        updated_content = updated_content.replace(f'ES_URL: http://elasticsearch-{instance_id}:9200', 
                                                f'ES_URL: http://localhost:{elasticsearch_port}')
        
        with open(compose_file, 'w') as f:
            f.write(updated_content)
        
        # Restart MCP server and AI agent with the new configuration
        cmd = ['docker', 'compose', '-f', compose_file, 'restart', f'mcp-server-{instance_id}', f'doc-agent-api-{instance_id}']
        result = subprocess.run(cmd, capture_output=True, text=True)
        
        if result.returncode == 0:
            print(f"Successfully updated MCP server with API key for {instance_id}")
...
def deploy_application():
    """Deploy a new instance of the application with dynamic ports"""
    try:
        # Get request data
        ...
        deployment_info = {
            ...
            'endpoints': {
                'elasticsearch': f'http://localhost:{elasticsearch_port}',
                'mcp_server': f'http://localhost:{mcp_port}',
                'ai_agent': f'http://localhost:{ai_agent_port}'
            }
        }
...
if __name__ == '__main__':
    ...
    print("  POST /deploy - Deploy new application instance")
